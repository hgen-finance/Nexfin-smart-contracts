...
use {crate::error::NexfinError, spl_math::precise_number::PreciseNumber, std::fmt::Debug};
...
/// Helper function for mapping to NexfinError::CalculationFailure
...
pub trait CurveCalculator: Debug + DynPack {
    ...
    fn validate(&self) -> Result<(), NexfinError>;
    ...
    fn validate_supply(&self, token_a_amount: u64, token_b_amount: u64) -> Result<(), NexfinError> {
        if token_a_amount == 0 {
            return Err(NexfinError::EmptySupply);
        }
        if token_b_amount == 0 {
            return Err(NexfinError::EmptySupply);
        }
